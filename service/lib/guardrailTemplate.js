...
// parent_sector is stored as an internal code (it keys the sector
// thresholds); generated text spells it out. The ingestion default
// "Uncategorized" says nothing, so it is written as "not sector-specific".
const SECTOR_LABEL = {
...
function sectorLabel(code) {
  if (!code || code === "Uncategorized") return "not sector-specific";
...
export function buildGuardrailTemplate(useCase, regulations) {
  ...
  const sector = sectorLabel(useCase.parent_sector);
...
# Sector: ${sector} | Modality: ${useCase.modality} | Risk tier: ${useCase.risk_tier || "unclassified"}
#
...
Auto-generated guardrail repository for **${useCaseLabel}** (${sector}, ${useCase.modality}).
