...
To evaluate the Dispatch flow without a personal GitHub account, the flow
can be evaluated by reading the code path in app/lib/githubClient.js, and
by inspecting real example output already generated by this exact flow:
https://github.com/complianceguardrails-source/guardrail-front-office-voice-agent-mortgage-escalation-support
```

## Screenshots

iPhone-only (`supportsTablet` is disabled -- no iPad screenshots needed).
Captured directly from a real iPhone 15 Pro Max (6.7" bucket, 1290x2796px)
via Expo Go running against production data: Radar, Use Cases, Audit (with
the detail pane expanded showing the policy diff and reference-repo card),
and Horizon.
